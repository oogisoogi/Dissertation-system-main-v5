...
- **시뮬레이션 모드**: Quick(20-30쪽) / Full(150쪽+) / Both / Smart 필수 선택 (결정론적 Python 라우팅)
...
| 결정론적 시뮬레이션 라우팅 | `simulation_router.py`로 LLM 없이 실행 경로 결정 |
| 시뮬레이션 모드 필수 선택 | Quick/Full/Both/Smart 중 사용자 명시적 선택 필수 |
...
| 핵심 스크립트     | 56개  | 검증, 게이트 관리, 품질 평가, 시뮬레이션 라우팅 등 |
...
Phase 3: 논문작성
  └─ 시뮬레이션 모드 라우팅 (결정론적) → 아웃라인 설계 → 챕터별 집필 → DWC 검증 → 품질검토
...
.claude/
  ...
  ├─ skills/              # 핵심 스킬 및 오케스트레이터 (56개 스크립트)
